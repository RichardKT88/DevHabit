...
public sealed class CreateHabitDtoValidator : AbstractValidator<CreateHabitDto>
{
    ...
    {
        string normlizedUnit = unit.ToLowerInvariant();
        return type switch
        {
            // Binary habits should only use count-based units
            HabitType.Binary => AllowedUnitsForBinaryHabits.Contains(normlizedUnit),
            // Measurable habits can use any of the allowed units
            HabitType.Measurable => AllowedUnits.Contains(normlizedUnit),
